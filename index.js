  ...
  (0, _createClass3.default)(Boundingbox, [{
    ...
    value: function componentDidMount() {
      ...
      background.onload = function () {
        ...
        _this2.canvas.onclick = function (e){
          ...
          //_this2.props.onSelected('hovered');
...
Boundingbox.defaultProps = {
  ...
  drawBox: function drawBox(canvas, box, color, lineWidth) {
    ...
    ctx.beginPath();
    ctx.moveTo(x + thirtyPercent, y);
    ctx.lineTo(x, y);
    ctx.lineTo(x, y + fixedHeight);
    ctx.lineTo(x + thirtyPercent, y + fixedHeight);
    ctx.stroke();

    // Right segment
    ctx.beginPath();
    ctx.moveTo(x + seventyPercent, y);
    ctx.lineTo(x + width, y);
    ctx.lineTo(x + width, y + fixedHeight);
    ctx.lineTo(x + seventyPercent, y + fixedHeight);
    ctx.stroke();
  },
